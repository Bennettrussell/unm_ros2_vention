...
class VentionNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('use_drive_1', True)
        ...
        self.use_drive_1 = bool(self.get_parameter('use_drive_1').value)
        ...
        # ------------------------------------------------------
        self.pub_drive_1_pos = self.create_publisher(Float32, 'drive_1_position', 10)
        ...
        self.sub_drive_1_vel_cmd = self.create_subscription(
            Float32,
            'drive_1_vel_cmd',
            self.cb_drive_1_vel_cmd,
            10
        )
        ...
        self.sub_drive_1_pos_cmd = self.create_subscription(
            Float32,
            'drive_1_pos_cmd',
            self.cb_drive_1_pos_cmd,
            10
        )
    ...
    # Velocity Callbacks
    # ------------------------------------------------------
    def cb_drive_1_vel_cmd(self, msg: Float32):
        if not self.use_drive_1:
            return
        self.vel_drive_1 = msg.data
        self.get_logger().debug(f"[Drive1] velocity => {self.vel_drive_1} mm/s")
    ...
    # Position Callbacks
    # ------------------------------------------------------
    def cb_drive_1_pos_cmd(self, msg: Float32):
        if not self.use_drive_1:
            return
        position = msg.data
        self.get_logger().info(f"[Drive1] position cmd => {position} mm")
        try:
            self.vention.move_to_position(axis=1, position=position)
        except Exception as e:
            self.get_logger().error(f"[Drive1] position move error: {e}")
        ...
        if self.use_drive_1:
            try:
                if abs(self.vel_drive_1) > 1e-6:
                    self.vention.move_continuous(axis=1, speed_mm_s=self.vel_drive_1, accel_mm_s2=100.0)
                else:
                    self.vention.stop_move_continuous(axis=1, accel_mm_s2=100.0)
            except Exception as e:
                self.get_logger().error(f"[Drive1] velocity update error: {e}")
